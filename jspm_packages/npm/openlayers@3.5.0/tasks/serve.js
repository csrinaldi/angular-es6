...
(function(process) {
  ...
  var createServer = exports.createServer = function(callback) {
    ...
    var manager = new closure.Manager({
      lib: ['src/**/*.js', 'build/ol.ext/*.js', 'test/spec/**/*.test.js', 'test_rendering/spec/**/*.test.js', 'build/test_requires.js', 'build/test_rendering_requires.js'],
      ...
      server = new closure.Server({
        ...
        getMain: function(req) {
          ...
          if (pathName.indexOf('/test/') === 0) {
            main = path.resolve(path.join(process.cwd(), 'build'), 'test_requires.js');
          } else if (pathName.indexOf('/test_rendering/') === 0) {
            main = path.resolve(path.join(process.cwd(), 'build'), 'test_rendering_requires.js');
          } else {
            if (query.id) {
              if (referer) {
                var from = path.join(process.cwd(), path.dirname(url.parse(referer).pathname));
    ...
    buildExamplesOrFatal(function() {
      ...
      createServer(function(err, server) {
        ...
        server.listen(options.port, function() {
          log.info('serve', 'Listening on http://localhost:' + options.port + '/ (Ctrl+C to stop)');
        });
