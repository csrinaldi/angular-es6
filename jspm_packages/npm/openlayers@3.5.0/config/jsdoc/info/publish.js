...
(function(process) {
  ...
  exports.publish = function(data, opts) {
    ...
    var classes = {};
    var docs = data([{define: {isObject: true}}, function() {
      if (this.kind == 'class') {
      ...
      return (typeof this.api == 'string' || this.meta && (/[\\\/]externs$/).test(this.meta.path));
    }], {kind: {'!is': 'file'}}, {kind: {'!is': 'event'}}).get();
    var symbols = [];
    var defines = [];
    var typedefs = [];
    var externs = [];
    ...
    docs.filter(function(doc) {
      ...
      if (constructor && constructor.substr(-1) === '_') {
        assert.strictEqual(doc.inherited, true, 'Unexpected export on private class: ' + doc.longname);
        include = false;
      }
      return include;
      ...
      if (isExterns && doc.longname.indexOf('olx.') === 0) {
        if (doc.kind == 'typedef') {
          ...
          typedef.types[0] = type.replace(/\}$/, ', ' + doc.longname.split('#')[1] + ': (' + getTypes(doc.type.names).join('|') + ')}').replace('{, ', '{');
        }
      } else if (doc.define) {
        defines.push({
          name: doc.longname,
        ...
        if (doc.returns) {
          symbol.returns = {types: getTypes(doc.returns[0].type.names)};
        ...
        if (doc.api && symbol.extends) {
          while (symbol.extends in classes && !classes[symbol.extends].api && classes[symbol.extends].augments) {
